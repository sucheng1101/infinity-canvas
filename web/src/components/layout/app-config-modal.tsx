...
import { Cloud, Download, KeyRound, RefreshCw, Upload, Wifi } from "lucide-react";
...
import { hasAnyChannelApiKey, normalizeAiConfig, useConfigStore, type AiConfig, type ConfigTabKey, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const finishConfig = () => {
        const ready = config.channels.some((channel) => channel.baseUrl.trim() && hasAnyChannelApiKey(channel) && channel.models.length);
                                        ...
                                        <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800">
                                            <div className="min-w-0">
                                                ...
                                                <div className="mt-1 truncate text-xs text-stone-500">
                                                    OpenAI · {t("config.channels.modelCount", { count: channel.models.length })} · {channel.baseUrl}
                                                </div>
                                            </div>
                                            <div className="flex shrink-0 gap-2">
                                                <Button size="small" icon={<KeyRound className="size-3.5" />} onClick={() => setEditingChannelId(channel.id)}>
                                                    {t("common.edit")}
                                                </Button>
                                            </div>
                                        </div>
                                ...
                                <div className="mb-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                                    {modelGroups.map((group) => (
                                        <Form.Item key={group.modelKey} label={t(group.labelKey)} className="mb-0">
                                            <ModelPicker config={config} value={config[group.modelKey]} onChange={(model) => updateConfig(group.modelKey, model)} capability={group.capability} fullWidth showChannel={false} />
                                        </Form.Item>
                                    ))}
                                </div>
...
function withChannels(config: AiConfig, channels: ModelChannel[]): AiConfig {
    return normalizeAiConfig({ ...config, channels });
}
