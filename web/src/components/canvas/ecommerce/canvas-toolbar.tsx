...
import { CircleDot, Eraser, Grid2x2, Group, Hand, Image as ImageIcon, Info, LayoutGrid, MessageSquare, Moon, MousePointer2, Music2, Palette, Play, Puzzle, Redo2, Settings2, Square, Sun, Trash2, Type, Undo2, Upload, Video } from "lucide-react";
    ...
    return (
        <div ref={rootRef} className="pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }}>
            ...
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
                ...
                <ToolbarButton id="tool-tidy" label={t("canvas.toolbar.tidy")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onTidy}>
                    <LayoutGrid className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-generate-set" label={t("canvas.toolbar.generateSet")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onGenerateSet}>
                    <Play className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-prompt-chat" label={t("canvas.toolbar.promptChat")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onPromptChat}>
...
function toolLabel(id: string, t: (key: string) => string) {
    ...
    if (id === "tool-tidy") return t("canvas.toolbar.tidy");
    if (id === "tool-generate-set") return t("canvas.toolbar.generateSet");
    if (id === "tool-prompt-chat") return t("canvas.toolbar.promptChat");
