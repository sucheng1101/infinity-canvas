export default {
    ...
    config: {
        ...
        channels: {
            description: "渠道固定为 codex.helpapis.com 和 helpapis.com；每个渠道支持通用 Key 和按能力分配的专用 Key，可继续拉取模型、指定能力和配置调用脚本。",
            add: "新增渠道",
            unnamed: "未命名渠道",
        ...
        channelEditor: {
            ...
            defaultApiKeyDescription: "用于拉取模型，并作为各能力未配置专用 Key 时的默认凭据。",
            capabilityApiKeys: "能力专用 Key",
            capabilityApiKey: "{{capability}} Key",
            capabilityApiKeyPlaceholder: "留空则使用通用 Key",
            capabilityApiKeysDescription: "请求会根据模型能力优先使用对应 Key；适合图片 Key 与文本 Key 权限不同的渠道。",
            models: "渠道模型",
