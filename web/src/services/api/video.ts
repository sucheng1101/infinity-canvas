...
import { videoResolutionRequestValue } from "@/lib/video-model-presets";
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    const resolutionValue = videoResolutionRequestValue(modelName, config.vquality);
    body.append("model", modelName);
    body.append("prompt", prompt);
    body.append("seconds", normalizeVideoSeconds(config.videoSeconds));
    body.append("size", normalizeVideoSize(config.size, resolutionValue) || "1280x720");
    body.append("resolution_name", resolutionValue);
