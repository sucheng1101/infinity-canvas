...
import { buildApiUrl, resolveApiBaseUrl, withLocalProxy, type AiConfig, type ModelCapability } from "@/stores/use-config-store";
...
function pluginUrl(config: AiConfig, path: string) {
    if (/^https?:/i.test(path)) return withLocalProxy(resolveApiBaseUrl(path));
