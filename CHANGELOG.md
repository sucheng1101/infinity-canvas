...
+ [新增] 画布图片节点信息显示请求质量，并在接口回传质量不一致时标注实际质量，便于排查渠道降级。

## v0.19.6 - 2026-09-18

+ [新增] 普通无限画布增加“一键整理图片”，仅将未分组图片按网格排列，其他节点保持原位并支持撤销。

## v0.19.5 - 2026-09-18

+ [合并] 吸收官方 v0.19.0 的 WebP 缩略图、图片请求超时、ZCode 插件、画布拖拽与连线渲染优化。
+ [新增] 图片生成支持 8K 分辨率和自定义短边、宽高比，尺寸上限同步放宽到最长边 7680px。
+ [新增] MiniMax H3 视频模型支持 768、1080p、2K、4K 原生分辨率参数。
+ [保留] 电商画布、生产直连代理、生成失败诊断和项目专属发布流程继续保留。
+ [新增] 视频分辨率按模型适配：MiniMax H3 视频模型显示其专属分辨率档位（768 / 1080p / 2K / 4K），请求按原生取值发送；其他模型继续使用默认 480p / 720p / 1080p 档位。
+ [新增] 画布生图新增 8K 分辨率档位，并支持自定义短边像素与自定义宽高比；图片尺寸上限同步放宽到最长边 7680px、总像素 58982400。

## v0.18.4

+ [新增] 新增电商提示词与工作流调研文档，整理商品事实、平台适配、套图规划、QA、许可边界及 Infinite Canvas 分阶段落地建议。
+ [新增] 在无限画布项目库增加电商项目入口，支持录入商品资料并预置商品资料、套图规划、三类生成节点和 QA 流程。
+ [调整] 合并图片与视频工作台为可切换的创作工作台，并新增独立项目管理入口，普通无限画布继续保留。
+ [新增] 电商项目支持上传多张参考图并连接到生成流程，三类生成节点默认加入实拍真实感提示词。
+ [优化] 新建电商项目弹窗改为 AI 对话式：填商品名称、传参考图后一键生成完整方案（类目、人群、卖点与三类提示词），方案区常驻可手动填写、AI 生成即自动填充，可连续对话调整；类目/人群/卖点支持内置选项与自由输入，场景模板为名称行内的下拉选择；高级选项常驻一行紧凑排列，可分别选择 AI 方案使用的文本模型与生成模型，上传参考图改为方形加号。
+ [优化] 项目管理与我的画布彻底分离：项目管理成为独立的电商项目列表页（新建弹窗随之迁入），我的画布不再显示电商项目，普通画布与电商项目互不混列。
+ [调整] 所有用户的默认偏好模型统一为：默认文本模型 gpt-5.6-sol、默认生图模型 gemini-3.1-flash-image-preview，已有配置升级时一次性重置为新默认，之后仍可在偏好设置中自行修改。
+ [文档] 新增 .agents/skills/ecommerce-canvas-flow skill，固化电商流程的产品路线、提示词格式与隔离约定。
+ [优化] 电商画布新增「AI 改词」对话面板，可直接用对话修改三类节点的组合提示词；节点保留独立开始生成按钮可单独生成，「生成套图」打开生成面板，集中展示完整提示词与参考图并一键生成（含 QA 检查）。
+ [新增] 新建电商项目支持场景模板预设，选中场景会写入商品资料，并影响 AI 提示词生成与场景图提示词。
+ [新增] 电商模板 QA 节点升级为可运行的检查配置：三类图生成后点击开始生成，AI 会结合商品资料、套图规划与三张生成图输出逐图的通过项、问题项和修订建议。
+ [优化] 电商画布工具栏新增「生成套图」：一键按顺序生成三张商品图并自动运行 QA 检查；框选批量生成同样保证 QA 节点最后执行。
+ [新增] 电商项目画布独立入口 /projects/:id：多选批量生成、一键整理排版、生成结果避让放置、节点自定义标题和失败报错明细只在电商画布生效，原有无限画布行为保持基线不变。
+ [文档] 电商调研文档补充 AnyDoor、OOTDiffusion 与主流商拍产品调研，新增七条画布优化建议。

## v0.18.3

+ [新增] 图片工作台生成日志持久化失败诊断与单张尝试明细，支持从历史记录查看并复制错误信息。
+ [兼容] 在官方 v0.18.0 基线上融合项目 v0.17.1 至 v0.17.3 的固定渠道、模型专用 Key、生产直连和 HEIC/HEIF 支持。
...
+ [修复] 通过 URL 导入 API 凭据时仅更新 Base URL 匹配的固定渠道，不再覆盖渠道列表第一项；缺少、非法或非固定 Base URL 时不修改配置。

## v0.17.3 - 2026-09-05

+ [新增] 图片导入支持 HEIC/HEIF，统一转换为浏览器可预览的 JPEG 后再保存和发送。

## v0.17.2 - 2026-09-04

+ [优化] 两个固定 AI 渠道在生产站点改用 HTTPS 同源直连入口，绕过 Cloudflare 链路超时且保持界面渠道域名不变。

## v0.17.1 - 2026-09-03

+ [新增] 增加 GitHub 与 Gitee 双仓一致性校验、静态站备份切换、验收回滚和清理要求的生产发布 SOP。
+ [优化] 模型专用 Key 输入新增重复检测和提示，存在重复项时阻止批量同步与保存。
+ [优化] 一键获取全部 Key 模型时以最新结果重建渠道模型列表，并清理失效 Key 或已下架模型留下的旧绑定。
...
+ [新增] 固定 AI 渠道支持通用 Key 与任意数量的模型专用 Key，每条 Key 可独立拉取支持列表并手动绑定模型，未绑定模型回退通用 Key。
+ [调整] 全部模型入口改为先选渠道、再选该渠道汇总模型的两级控件，切换渠道时自动匹配当前生成能力。
+ [调整] 新配置不再预置固定模型，需由渠道 Key 获取或手动添加模型后选择。
+ [调整] 画布生图默认比例改为 3:4，默认张数改为 1 张，并迁移仍使用旧默认值的本地配置。
+ [调整] 移除应用内 GitHub 与文档入口，AI 渠道固定为 codex.helpapis.com 和 helpapis.com 且仅开放 Key 与模型配置。
