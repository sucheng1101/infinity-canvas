...
- 本项目是在官方无限画布基础上复制开发的，新增功能不得改变或破坏原有通用画布的行为与界面。电商画布增强能力走独立入口 `/projects/:id`（`pages/canvas/ecommerce-project.tsx` 传 `canvasKind="ecommerce"`），工具栏、选区工具栏、生成配置面板使用 `components/canvas/ecommerce/` 下的副本组件；共享画布文件里所有增强必须以 `isEcommerce` 门控，原画布分支保持基线表达式，纯新增导出（工具函数、i18n 文案等）可不门控。修改共享画布代码前先确认对原画布零行为变化。电商流程的产品路线、提示词格式与实现约定见 `.agents/skills/ecommerce-canvas-flow/SKILL.md`，改动电商相关功能前先读该文件。
